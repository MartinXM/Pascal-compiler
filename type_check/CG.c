...
void CGExpOp(TreeNode* pnode){
	...
	if (pnode->child[0]!=NULL && pnode->child[1]!=NULL){
		...
		if ((pnode->child[0])->RunningType==EXPTYPE_INT && (pnode->child[1])->RunningType==EXPTYPE_INT)
	...
	if (pnode->RunningType==EXPTYPE_INT){
		CG_OUTPUT("pop ebx\n");
		CG_OUTPUT("pop eax\n");
...
//	if (pnode->child[0]!=NULL)
	...
	while (tt!=NULL){
		...
		if (tt->RunningType==EXPTYPE_REAL){
			CG_OUTPUT("pusha\n");
			CG_OUTPUT("push eax\n");
			CG_OUTPUT("fld dword ptr [esp]\n");
			CG_OUTPUT("sub esp, 4\n");
		...
		else if (tt->RunningType==EXPTYPE_INT){
			CG_OUTPUT("pusha\n");
			if  (pnode->attr.op==TOKEN_WRITELN){
...
//	if (pnode->child[0]!=NULL)
	...
	while (tt!=NULL){
		...
		if (tt->RunningType==EXPTYPE_INT)
			strcat(output_all,"int, addr lb_tmp\n");
		else if (tt->RunningType==EXPTYPE_REAL)
